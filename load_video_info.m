...
%   http://www.isr.uc.pt/~henriques/
	...
	if size(ground_truth,1) == 1,
		...
		ground_truth = [ground_truth(:,[2,1]) + ground_truth(:,[4,3]) / 2, ground_truth(:,4), ground_truth(:,3)];
	end
	...
	if isempty(idx),
        ...
        elseif exist(sprintf('%simg%05i.png', video_path, frames{idx,2}), 'file'),
			img_files = num2str((frames{idx,2} : frames{idx,3})', 'img%05i.png');
